...
bool WaylandNoGUIPlatform::Initialize()
{
	...
	if (!m_xkb_context)
	{
		pxFailRel("Failed to create XKB context");
		return false;
	}
	...
	if (!m_display)
	{
		pxFailRel("Failed to connect to Wayland display.");
		return false;
	}
	...
	{
		pxFailRel("Missing Wayland shell/compositor\n");
		return false;
	}
...
bool WaylandNoGUIPlatform::CreatePlatformWindow(std::string title)
{
	...
	{
		Console.Error("Failed to create compositor/shell surfaces");
		return false;
	}
...
std::optional<u32> WaylandNoGUIPlatform::ConvertHostKeyboardStringToCode(const std::string_view& str)
{
	...
	{
		if (StringUtil::compareNoCase(it.second, str))
			return it.first;
	}
...
void WaylandNoGUIPlatform::GlobalRegistryHandler(void* data, wl_registry* registry, uint32_t id,
...
void WaylandNoGUIPlatform::XDGSurfaceConfigure(void* data, struct xdg_surface* xdg_surface,
	uint32_t serial)
{
	xdg_surface_ack_configure(xdg_surface, serial);
}

void WaylandNoGUIPlatform::TopLevelConfigure(void* data, struct xdg_toplevel* xdg_toplevel,
...
void WaylandNoGUIPlatform::SeatCapabilities(void* data, wl_seat* seat, uint32_t capabilities)
{
	...
	if (capabilities & WL_SEAT_CAPABILITY_KEYBOARD)
	{
		static const wl_keyboard_listener keyboard_listener = {
			&WaylandNoGUIPlatform::KeyboardKeymap, &WaylandNoGUIPlatform::KeyboardEnter,
			&WaylandNoGUIPlatform::KeyboardLeave, &WaylandNoGUIPlatform::KeyboardKey,
			&WaylandNoGUIPlatform::KeyboardModifiers};
...
void WaylandNoGUIPlatform::KeyboardKeymap(void* data, wl_keyboard* keyboard, uint32_t format, int32_t fd,
	uint32_t size)
{
	WaylandNoGUIPlatform* platform = static_cast<WaylandNoGUIPlatform*>(data);
	...
	platform->m_xkb_keymap = xkb_keymap_new_from_string(platform->m_xkb_context, keymap_string, XKB_KEYMAP_FORMAT_TEXT_V1, XKB_KEYMAP_COMPILE_NO_FLAGS);
...
void WaylandNoGUIPlatform::InitializeKeyMap()
{
	...
	pxAssert(max_keycode >= min_keycode);
...
void WaylandNoGUIPlatform::KeyboardEnter(void* data, wl_keyboard* keyboard, uint32_t serial,
	wl_surface* surface, wl_array* keys)
{
}

void WaylandNoGUIPlatform::KeyboardLeave(void* data, wl_keyboard* keyboard, uint32_t serial,
	wl_surface* surface)
{
}

void WaylandNoGUIPlatform::KeyboardKey(void* data, wl_keyboard* keyboard, uint32_t serial, uint32_t time,
...
void WaylandNoGUIPlatform::PointerEnter(void* data, wl_pointer* pointer, uint32_t serial,
	wl_surface* surface, wl_fixed_t surface_x, wl_fixed_t surface_y)
{
}

void WaylandNoGUIPlatform::PointerLeave(void* data, wl_pointer* pointer, uint32_t serial,
	wl_surface* surface)
{
}

void WaylandNoGUIPlatform::PointerMotion(void* data, wl_pointer* pointer, uint32_t time, wl_fixed_t x,
	wl_fixed_t y)
{
...
void WaylandNoGUIPlatform::PointerAxis(void* data, wl_pointer* pointer, uint32_t time, uint32_t axis,
	wl_fixed_t value)
{
