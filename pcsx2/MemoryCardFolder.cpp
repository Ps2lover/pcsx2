...
void FolderMemoryCard::Open(std::string fullPath, const Pcsx2Config::McdOptions& mcdOptions, const u32 sizeInClusters, const bool enableFiltering, std::string filter, bool simulateFileWrites)
{
	...
	m_folderName = Path::NormalizeStdString(fullPath);
...
void FolderMemoryCard::LoadMemoryCardData(const u32 sizeInClusters, const bool enableFiltering, const std::string& filter)
{
	...
	const std::string superBlockFileName(Path::CombineStdString(m_folderName, "_pcsx2_superblock"));
...
static bool FilterMatches(const std::string& fileName, const std::string& filter)
{
	size_t start = 0;
	...
	if (FileSystem::DirectoryExists(dirPath.c_str()))
	{
		...
		for (const auto& file : GetOrderedFiles(dirPath))
		{
			...
			else
			{
				...
				// is a subdirectory
				const std::string filePath(Path::CombineStdString(dirPath, file.m_fileName));
				...
				// set metadata
				const std::string metaFileName(Path::CombineStdString(Path::CombineStdString(dirPath, "_pcsx2_meta_directory"), file.m_fileName));
...
{
	const std::string filePath(Path::CombineStdString(dirPath, fileEntry.m_fileName));
	...
	if (auto file = FileSystem::OpenManagedCFile(filePath.c_str(), "rb"); file)
	{
		...
		std::string metaFileName(Path::CombineStdString(Path::CombineStdString(dirPath, "_pcsx2_meta"), fileEntry.m_fileName));
		if (auto metaFile = FileSystem::OpenManagedCFile(metaFileName.c_str(), "rb"); metaFile)
		{
...
u32 FolderMemoryCard::CalculateRequiredClustersOfDirectory(const std::string& dirPath) const
{
	...
	for (const FILESYSTEM_FIND_DATA& fd : files)
	{
		...
		{
			requiredClusters += CalculateRequiredClustersOfDirectory(Path::CombineStdString(dirPath, fd.FileName));
		}
	}
		...
		if (entry->IsValid() && entry->IsUsed())
		{
			if (entry->IsDir())
			{
				if (!entry->IsDotDir())
				{
					...
					const std::string subDirPath(Path::CombineStdString(dirPath, cleanName));

					if (m_performFileWrites)
					{
						// if this directory has nonstandard metadata, write that to the file system
						const std::string fullSubDirPath(Path::CombineStdString(m_folderName, subDirPath));
						std::string metaFileName(Path::CombineStdString(fullSubDirPath, "_pcsx2_meta_directory"));
						...
						metaFileName = Path::CombineStdString(fullSubDirPath, "_pcsx2_index");
			...
			else if (entry->IsFile())
			{
				...
				if (entry->entry.data.length == 0)
				{
					...
					if (m_performFileWrites)
					{
						...
						const std::string fullDirPath(Path::CombineStdString(m_folderName, dirPath));
						const std::string fn(Path::CombineStdString(fullDirPath, cleanName));
				...
				if (m_performFileWrites)
				{
					std::string fullDirPath(Path::CombineStdString(m_folderName, dirPath));
					FileAccessHelper::WriteIndex(std::move(fullDirPath), entry, parent);
				}
			}
		}
	}
	...
	for (auto it = oldFileEntries.cbegin(); it != oldFileEntries.cend(); ++it)
	{
		...
		if (entry->IsValid() && entry->IsUsed() && !entry->IsDotDir())
		{
			...
			if (newEntry == nullptr)
			{
				...
				FileAccessHelper::CleanMemcardFilename(cleanName);
				const std::string fullDirPath(Path::CombineStdString(m_folderName, dirPath));
				const std::string filePath(Path::CombineStdString(fullDirPath, cleanName));
				m_lastAccessedFile.CloseMatching(filePath);
				const std::string newFilePath(Path::CombineStdString(Path::CombineStdString(m_folderName, dirPath), fmt::format("_pcsx2_deleted_{}", cleanName)));
...
{
	const std::string indexName(Path::CombineStdString(filePath, "_pcsx2_index"));
...
wxFFile* FileAccessHelper::Open(const std::string_view& folderName, MemoryCardFileMetadataReference* fileRef, bool writeMetadata /* = false */)
{
...
void FileAccessHelper::WriteIndex(std::string folderName, MemoryCardFileEntry* const entry, MemoryCardFileMetadataReference* const parent)
{
...
bool MemoryCardFileMetadataReference::GetPath(std::string* fileName) const
{
	...
	if (entry->IsDir() || entry->IsFile())
	{
		*fileName = Path::CombineStdString(*fileName, cleanName);
	}
