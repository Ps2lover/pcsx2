...
namespace FullscreenUI
{
	...
	static void DrawIntListSetting(const char* title, const char* summary, const char* section, const char* key, int default_value,
		...
		float height = ImGuiFullscreen::LAYOUT_MENU_BUTTON_HEIGHT,
		ImFont* font = g_large_font, ImFont* summary_font = g_medium_font);
	static void DrawStringListSetting(const char* title, const char* summary, const char* section, const char* key, const char* default_value,
		const char* const* options, const char* const* option_values, size_t option_count, bool enabled = true,
		float height = ImGuiFullscreen::LAYOUT_MENU_BUTTON_HEIGHT,
		ImFont* font = g_large_font, ImFont* summary_font = g_medium_font);
...
bool FullscreenUI::Initialize()
{
	...
	{
		UpdateGameDetails(VMManager::GetDiscPath(), VMManager::GetGameSerial(),
			VMManager::GetGameName(), VMManager::GetGameCRC());
	}
	else
	{
		SwitchToLanding();
...
{
	Host::RunOnCPUThread([]() {
		Host::RequestVMShutdown(EmuConfig.SaveStateOnShutdown);
	});
}
...
void FullscreenUI::DoChangeDiscFromFile()
{
	auto callback = [](const std::string& path) {
		if (!path.empty())
		{
			Host::RunOnCPUThread([path = path]() {
				VMManager::ChangeDisc(std::move(path));
			});
		}
	...
	OpenFileSelector(ICON_FA_COMPACT_DISC "  Select Disc Image", false, std::move(callback), GetDiscImageFilters(),
		std::string(FileSystem::GetPathDirectory(s_current_game_path)));
}
...
void FullscreenUI::DrawIntListSetting(const char* title, const char* summary, const char* section, const char* key,
	int default_value, const char* const* options, size_t option_count, int option_offset, bool enabled, float height,
...
void FullscreenUI::DrawStringListSetting(const char* title, const char* summary, const char* section, const char* key, const char* default_value, const char* const* options, const char* const* option_values, size_t option_count, bool enabled, float height, ImFont* font, ImFont* summary_font)
{
	SettingsInterface* bsi = Host::Internal::GetBaseSettingsLayer();
...
void FullscreenUI::DrawInterfaceSettingsPage()
{
	...
	DrawToggleSetting(
		"Double-Click Toggles Fullscreen", "Switches between full screen and windowed when the window is double-clicked.",
...
{
	static constexpr char* ee_cycle_rate_settings[] = {
		"50% Speed", "60% Speed", "75% Speed", "100% Speed (Default)", "130% Speed", "180% Speed", "300% Speed"};
	static constexpr char* ee_cycle_skip_settings[] = {
		"Normal (Default)", "Mild Underclock", "Moderate Overclock", "Maximum Overclock"};
	static constexpr char* ee_rounding_mode_settings[] = {
		"Nearest", "Negative", "Positive", "Chop/Zero (Default)"};
	static constexpr char* ee_clamping_mode_settings[] = {
		"None", "Normal (Default)", "Extra + Preserve Sign", "Full"};
	static constexpr char* vu_clamping_mode_settings[] = {
		"None", "Normal (Default)", "Extra", "Extra + Preserve Sign"};

	BeginMenuButtons();

	MenuHeading("Emotion Engine (MIPS-III/MIPS-IV)");
	DrawIntListSetting("Cycle Rate", "Underclocks or overclocks the emulated Emotion Engine CPU.",
		"EmuCore/Speedhacks", "EECycleRate", 0, ee_cycle_rate_settings, std::size(ee_cycle_rate_settings), -3);
	DrawIntListSetting("Cycle Skip", "Adds a penalty to the Emulated Emotion Engine for executing VU programs.",
		"EmuCore/Speedhacks", "EECycleSkip", 0, ee_cycle_skip_settings, std::size(ee_cycle_skip_settings));
	DrawIntListSetting("Rounding Mode", "TODO",
		"EmuCore/CPU", "FPU.Roundmode", 3, ee_rounding_mode_settings, std::size(ee_rounding_mode_settings));
	DrawIntListSetting("Clamping Mode", "TODO",
		"EmuCore/CPU", "FPU.Clampmode", 1, ee_clamping_mode_settings, std::size(ee_clamping_mode_settings));
	DrawToggleSetting("Enable Recompiler", "Enables recompilation of MIPS code.", "EmuCore/CPU/Recompiler", "EnableEE", true);
	...
	DrawIntListSetting("Rounding Mode", "TODO",
		"EmuCore/CPU", "VU.Roundmode", 3, ee_rounding_mode_settings, std::size(ee_rounding_mode_settings));
	DrawIntListSetting("Clamping Mode", "TODO",
		"EmuCore/CPU", "FPU.Clampmode", 1, vu_clamping_mode_settings, std::size(vu_clamping_mode_settings));
	DrawToggleSetting("Enable MTVU (Multi-Threaded VU1)", "TODO.", "EmuCore/Speedhacks", "vuThread", false);
	DrawToggleSetting("Enable Instant VU1", "TODO.", "EmuCore/Speedhacks", "vu1Instant", true);
	DrawToggleSetting("Enable VU0 Recompiler (Micro Mode)", "TODO.", "EmuCore/CPU/Recompiler", "EnableVU0", true);
	DrawToggleSetting("Enable VU1 Recompiler", "TODO.", "EmuCore/CPU/Recompiler", "EnableVU1", true);
	DrawToggleSetting("VU Flag Optimization", "TODO.", "EmuCore/Speedhacks", "vuFlagHack", true);

	MenuHeading("I/O Processor (MIPS-I)");
	DrawToggleSetting("Enable Fast CDVD", "TODO.", "EmuCore/Speedhacks", "fastCDVD", false);
	DrawToggleSetting("Enable Recompiler", "TODO.", "EmuCore/CPU/Recompiler", "EnableIOP", true);
...
{
	static constexpr char* s_renderer_names[] = {
		"Automatic",
#ifdef _WIN32
		"Direct3D 11",
		"Direct3D 12",
		...
		"Software",
		"Null"};
	static constexpr char* s_renderer_values[] = {
	...
	static constexpr char* s_deinterlacing_options[] = {
		"None",
		...
		"Automatic (Default)"};
	static constexpr char* s_resolution_options[] = {
		"Native (PS2)",
		"2x Native (~720p)",
		"3x Native (~1080p)",
		"4x Native (~1440p/2K)",
	...
	static constexpr char* s_mipmapping_options[] = {
		"Automatic (Default)",
		"Off",
		"Basic (Generated Mipmaps)",
		"Full (PS2 Mipmaps)"};
	static constexpr char* s_bilinear_options[] = {
		"Nearest",
		"Bilinear (Forced)",
		"Bilinear (PS2)",
		"Bilinear (Forced excluding sprite)"};
	static constexpr char* s_trilinear_options[] = {
		"Automatic (Default)",
		"Off (None)",
		"Trilinear (PS2)",
		"Trilinear (Forced)"};
	static constexpr char* s_dithering_options[] = {
		"Off",
		"Scaled",
		"Unscaled (Default)"};
	static constexpr char* s_crc_fix_options[] = {
		"Automatic (Default)",
		"None (Debug)",
		"Minimum (Debug)",
		"Partial (OpenGL)",
		"Full (Direct3D)",
		"Aggressive"};
	static constexpr char* s_blending_options[] = {
		"Minimum",
		"Basic (Recommended)",
		"Medium",
		"High",
		"Full (Slow)",
		"Maximum (Very Slow)"};
	static const char* s_anisotropic_filtering_entries[] = {"Off (Default)", "2x", "4x", "8x", "16x"};
	static const char* s_anisotropic_filtering_values[] = {"0", "2", "4", "8", "16"};
	static const char* s_preloading_options[] = {"None", "Partial", "Full (Hash Cache)"};

	SettingsInterface* bsi = Host::Internal::GetBaseSettingsLayer();

	const GSRendererType renderer = static_cast<GSRendererType>(bsi->GetIntValue("EmuCore/GS", "Renderer", static_cast<int>(GSRendererType::Auto)));
	const bool is_hardware = (renderer == GSRendererType::DX11 || renderer == GSRendererType::DX12 || renderer == GSRendererType::OGL || renderer == GSRendererType::VK || renderer == GSRendererType::Metal);
	const bool is_software = (renderer == GSRendererType::SW);

	BeginMenuButtons();

	MenuHeading("Renderer");
	DrawStringListSetting("Renderer", "Selects the API used to render the emulated GS.", "EmuCore/GS", "Renderer", "-1", s_renderer_names, s_renderer_values, std::size(s_renderer_names));
	DrawToggleSetting("Sync To Host Refresh (VSync)", "TODO", "EmuCore/GS", "VsyncEnable", false);

	MenuHeading("Display");
	DrawStringListSetting("Aspect Ratio", "TODO", "EmuCore/GS", "AspectRatio", "Auto 4:3/3:2", Pcsx2Config::GSOptions::AspectRatioNames, Pcsx2Config::GSOptions::AspectRatioNames, 0);
	DrawStringListSetting("FMV Aspect Ratio", "TODO", "EmuCore/GS", "FMVAspectRatioSwitch", "Auto 4:3/3:2", Pcsx2Config::GSOptions::FMVAspectRatioSwitchNames, Pcsx2Config::GSOptions::FMVAspectRatioSwitchNames, 0);
	DrawIntListSetting("Deinterlacing", "TODO", "EmuCore/GS", "deinterlace", static_cast<int>(GSInterlaceMode::Automatic), s_deinterlacing_options, std::size(s_deinterlacing_options));
	// Zoom
	// Stretch
	// Offset
	DrawToggleSetting("Bilinear Filtering", "TODO", "EmuCore/GS", "linear_present", true);
	DrawToggleSetting("Integer Upscaling", "TODO", "EmuCore/GS", "IntegerScaling", false);
	DrawToggleSetting("Internal Resolution Screenshots", "TODO", "EmuCore/GS", "InternalResolutionScreenshots", false);
	DrawToggleSetting("Screen Offsets", "TODO", "EmuCore/GS", "pcrtc_offsets", false);

	MenuHeading("Rendering");
	if (is_hardware)
	{
		DrawIntListSetting("Internal Resolution", "TODO", "EmuCore/GS", "upscale_multiplier", 1, s_resolution_options, std::size(s_resolution_options), 1);
		DrawIntListSetting("Mipmapping", "TODO", "EmuCore/GS", "mipmap_hw", static_cast<int>(HWMipmapLevel::Automatic), s_mipmapping_options, std::size(s_mipmapping_options), -1);
		DrawIntListSetting("Bilinear Filtering", "TODO", "EmuCore/GS", "filter", static_cast<int>(BiFiltering::PS2), s_bilinear_options, std::size(s_bilinear_options));
		DrawIntListSetting("Trilinear Filtering", "TODO", "EmuCore/GS", "UserHacks_TriFilter", static_cast<int>(TriFiltering::Automatic), s_trilinear_options, std::size(s_trilinear_options), -1);
		DrawStringListSetting("Anisotropic Filtering", "TODO", "EmuCore/GS", "MaxAnisotropy", "0",
			s_anisotropic_filtering_entries, s_anisotropic_filtering_values, std::size(s_anisotropic_filtering_entries));
		DrawIntListSetting("Dithering", "TODO", "EmuCore/GS", "dithering_ps2", 2, s_dithering_options, std::size(s_dithering_options));
		DrawIntListSetting("CRC Fix Level", "TODO", "EmuCore/GS", "crc_hack_level", static_cast<int>(CRCHackLevel::Automatic), s_crc_fix_options, std::size(s_crc_fix_options), -1);
		DrawIntListSetting("Blending Accuracy", "TODO", "EmuCore/GS", "accurate_blending_unit", static_cast<int>(AccBlendLevel::Basic), s_blending_options, std::size(s_blending_options));
		DrawIntListSetting("Texture Preloading", "TODO", "EmuCore/GS", "texture_preloading", static_cast<int>(TexturePreloadingLevel::Off), s_preloading_options, std::size(s_preloading_options));
		DrawToggleSetting("Accurate Destination Alpha Test", "TODO", "EmuCore/GS", "accurate_date", true);
		DrawToggleSetting("Conservative Buffer Allocation", "TODO", "EmuCore/GS", "conservative_framebuffer", true);
		DrawToggleSetting("GPU Palette Conversion", "TODO", "EmuCore/GS", "paltex", false);
	}
	else
	{
	}
	...
	if (is_hardware)
	{
		...
		if (manual_hw_fixes)
		{
			...
			// Skip Draw
			DrawToggleSetting("Auto Flush (Hardware)", "TODO", "EmuCore/GS", "UserHacks_AutoFlush", false, manual_hw_fixes);
			DrawToggleSetting("CPU Framebuffer Conversion", "TODO", "EmuCore/GS", "UserHacks_CPU_FB_Conversion", false, manual_hw_fixes);
			DrawToggleSetting("Disable Depth Support", "TODO", "EmuCore/GS", "UserHacks_DisableDepthSupport", false, manual_hw_fixes);
			DrawToggleSetting("Wrap GS Memory", "TODO", "EmuCore/GS", "wrap_gs_mem", false, manual_hw_fixes);
			DrawToggleSetting("Disable Safe Features", "TODO", "EmuCore/GS", "UserHacks_Disable_Safe_Features", false, manual_hw_fixes);
			DrawToggleSetting("Preload Frame", "TODO", "EmuCore/GS", "preload_frame_with_gs_data", false, manual_hw_fixes);
			DrawToggleSetting("Disable Partial Invalidation", "TODO", "EmuCore/GS", "UserHacks_DisablePartialInvalidation", false, manual_hw_fixes);
			...
			// Round Sprite
			// TC Offset X/Y
			DrawToggleSetting("Align Sprite", "TODO", "EmuCore/GS", "UserHacks_align_sprite_X", false, manual_hw_fixes);
			DrawToggleSetting("Merge Sprite", "TODO", "EmuCore/GS", "UserHacks_merge_pp_sprite", false, manual_hw_fixes);
	...
	{
		// extrathreads
		DrawToggleSetting("Auto Flush (Software)", "TODO", "EmuCore/GS", "autoflush_sw", true);
		DrawToggleSetting("Edge AA (AA1)", "TODO", "EmuCore/GS", "aa1", true);
		DrawToggleSetting("Mipmapping", "TODO", "EmuCore/GS", "mipmap", true);
	}


	MenuHeading("On-Screen Display");
	// OSD scale
	DrawToggleSetting("Show Messages", "TODO", "EmuCore/GS", "OsdShowMessages", true);
	DrawToggleSetting("Show Speed", "TODO", "EmuCore/GS", "OsdShowSpeed", false);
	DrawToggleSetting("Show FPS", "TODO", "EmuCore/GS", "OsdShowFPS", false);
	DrawToggleSetting("Show CPU Usage", "TODO", "EmuCore/GS", "OsdShowCPU", false);
	DrawToggleSetting("Show GPU Usage", "TODO", "EmuCore/GS", "OsdShowGPU", false);
	DrawToggleSetting("Show Resolution", "TODO", "EmuCore/GS", "OsdShowResolution", false);
	DrawToggleSetting("Show GS Statistics", "TODO", "EmuCore/GS", "OsdShowGSStats", false);
	DrawToggleSetting("Show Status Indicators", "TODO", "EmuCore/GS", "OsdShowIndicators", true);
	...
	if (BeginFullscreenColumnWindow(450.0f, LAYOUT_SCREEN_WIDTH, "game_list_entries"))
	{
		...
		for (const GameList::Entry* entry : s_game_list_sorted_entries)
		{
			...
			if (entry->serial.empty())
				summary.Write("%s - ", GameList::RegionToString(entry->region));
			else
				summary.Write("%s - %s - ", entry->serial.c_str(), GameList::RegionToString(entry->region));

			const std::string_view filename(FileSystem::GetFileNameFromPath(entry->path));
			summary.Write("%.*s", static_cast<int>(filename.size()), filename.data());
			...
			{
				ImGui::PushFont(g_medium_font);
				ImGui::RenderTextClipped(summary_bb.Min, summary_bb.Max, summary.c_str(), nullptr,
					nullptr, ImVec2(0.0f, 0.0f), &summary_bb);
				ImGui::PopFont();
			}
